...
def save_to_apple_notes(title, category, content):
    try:
        body = f"📰 {title}\n\n{content}\n\n_Categoría: {category}_"
        script = f'tell application "Notes" to tell account "iCloud" to make new note with properties {{name:"{title}", body:"{body}"}}'
...
HTML = open('/home/jmaudisio/dashboard/dashboard-template-v3.html').read()
...
@app.route('/api/apple-notes/save', methods=['POST'])
def api_apple_notes():
    data = request.get_json()
    title = data.get('title', '')
    ...
    success = save_to_apple_notes(title, category, content)
